...
from scripts.convert2jpeg import convert
...
class ExcelProcessor:
    ...
    @macro_with_new_excel_app(book="scope_v2/macro_scope_v2.xlsm")
    def create_scope_pdf_v2(self, macro_book, in_book, out_file_path, **kwargs):
        project = RequestAsProject(kwargs["request"])
        self.execute_macro(macro_book, "CreateScopeWithTemplate", "", project.name, "")

        return NO_DOWNLOAD  


    @macro_with_new_excel_app(book="purchase_order/macro_purchase_order.xlsm")
    def create_purchase_order(self, macro_book, in_book, out_file_path, **kwargs):
        project = RequestAsProject(kwargs["request"])
        self.execute_macro(macro_book, "CreatePurchaseOrderWithTemplate", "", project.name, "")
    ...
    def create_materials(self, macro_book, in_book, out_file_path, **kwargs):
        ...
        if len(excel_books):
            ...
            self.execute_macro(macro_book, "CreateMaterialsWithTemplate", db_path, project.name, materials_template, project.path)
        else:
            raise APPError("Missing CLIP books.")
            
    @macro_with_new_excel_app(book="customer_invoice/macro_customer_invoice.xlsm")
    def create_customer_invoice(self, macro_book, in_book, out_file_path, **kwargs):
        project = RequestAsProject(kwargs["request"])
        self.execute_macro(macro_book, "CreateCustomerInvoiceWithTemplate", "", project.name, "")

        return NO_DOWNLOAD            

    @staticmethod
    def image_compression(request: Request):
        """
        Сжатие изображений и упаковка в zip формат.
        Функция достаёт из запроса путь к проекту и работает с изображениями в папке Photos.
        """
        data = json.loads(request.data)[0]
        path_project = data.get(PROJECT_PATH_COL)
        path = os.path.join(path_project, "Photos")
        # path = "C:\\EcoSmart Solutions Team Folder\\1010 Elm\\Photos"
        save_path = os.path.join(path, "JPEG")
        debug(f"{path} start photo conversion.")
        response = convert(
            source_folder=path,
            target_folder=save_path,
        )
        if isinstance(response, str):
            debug(f"{response} end photo conversion.")
            return response
        debug(f"Exceptions: {response}")
        raise APPError("An error occurred while compressing")
